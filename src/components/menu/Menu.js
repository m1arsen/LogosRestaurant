    ...
    return items.map((item) => {
      ...
      return (
        <MenuPos
          ...
          key={item.id}
          data-tab={item.id}
          onClick={(e) => setopenedTab(e.target.dataset.tab)}
        >
          {item.name}
        </MenuPos>
      );
...
const View = ({ tabsNames, posSection }) => {
  return (
    <section className="menu">
      <MenuTop>
        <MenuTopContainer>{tabsNames}</MenuTopContainer>
      </MenuTop>

      <MenuPosContainer>{posSection}</MenuPosContainer>
    </section>
  );
};
